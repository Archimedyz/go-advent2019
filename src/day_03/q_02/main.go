...
func main() {
	...
	p1 := FindClosestIntersectionSteps(lines1.vertical, lines2.horizontal)
	p2 := FindClosestIntersectionSteps(lines2.vertical, lines1.horizontal)

	fmt.Printf("p1: %v\n", p1)
	fmt.Printf("p2: %v\n", p2)
}

func FindClosestIntersectionSteps(verticals []Pair, horizontals []Pair) int {
	min := -1
	for _, v := range verticals {
		for _, h := range horizontals {
			if Between(v.p1.x, h.p1.x, h.p2.x) { // vertical line is in range of the horizontal line
				if Between(h.p1.y, v.p1.y, v.p2.y) { // horizontal line is in range of the vertical line
					// intersection!
					distance := v.steps + h.steps + Abs(v.p1.y - h.p1.y) + Abs(h.p1.x- v.p1.x)
...
func Between(x int, v1 int, v2 int) bool {
	if v1 < v2 {
		return x >= v1 && x <= v2
	}
	return x >= v2 && x <= v1
