...
// Horizontal/Vertical lines, denoted by Pairs
type Lines struct {
	horizontal []Pair
	vertical   []Pair
}
...
func GetLines(wire string) Lines {
	result := Lines{}
	x, y := 0, 0
	...
	for _, v := range strings.Split(wire, ",") {
		...
		if v[0] == 'U' {
			...
			result.vertical = append(result.vertical, pair)
		} else if v[0] == 'D' {
			val, _ := strconv.Atoi(v[1:])

			pair := Pair{
				p1: Coord{x, y - val}, // deliberate flip here. want "bottom point" to be first
				p2: Coord{x, y},
			}
			...
			result.horizontal = append(result.horizontal, pair)
		} else { // v[0] == 'L'
			val, _ := strconv.Atoi(v[1:])

			pair := Pair{
				p1: Coord{x - val, y}, // deliberate flip here. want "left point" to be first
				p2: Coord{x, y},
			}
